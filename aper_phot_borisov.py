    ...
    for i in i_range:
        for j in j_range:
            ...
            if pos_cen_dist > r1 and pos_cen_dist <= r2:
                ...
                if img_data[i, j] == 0:
                    pixel_canvas += 1
    pixel = pixel - pixel_canvas
    pixel_unmask = pixel_unmask - pixel_canvas
    if method == 'mean':
        #return count, pixel
...
def azimuthal_ct(img_name,
    ...
    if not start:
        start = 0
    import math
    step_num = math.ceil((aperture-start)/step)
    count = 0
    pixel = 0
    err2 = 0
...
def aper_phot_bori(img_name, filt,
                   src_center, src_r,
                   bg_center, bg_r,
                   src_method, bg_method,
                   step=5, mask_img = False, start = False):
    '''
