...
def stack_info(evt_ind, filt, id_horizons):
    ...
    return time_list, x_list, y_list, time_array, x_comet, y_comet, exp_data, x_comet_exp, y_comet_exp, num, exp, hd

def sliceImg(time_list, x_list, y_list, x_comet, y_comet, time_array, i, size, filt, num, exp):
...
def sliceExp(exp_data,x_comet_exp,y_comet_exp,size,num,i):
    new_exp = set_coord(exp_data,
                        np.array([x_comet_exp[i]-1,y_comet_exp[i]-1]),
                        (size,size))
    slice_exp = new_exp/num
    return slice_exp
...
def stack(evt_ind, filt, id_horizons, size):
    time_list, x_list, y_list, time_array, x_comet, y_comet, exp_data, x_comet_exp, y_comet_exp, num, exp, hd = \
        stack_info(evt_ind, filt, id_horizons)
